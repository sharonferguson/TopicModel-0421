                ...
                for vi in range(vj+1, nTopWords): 
                    ...
                    for d in range(0,nDocs): 
                        ...
                        if (topic_word[t][vj] in coherence_corpus[d]) and (topic_word[t][vi] in coherence_corpus[d]): #check how many docs contain both word vj and vi
                            Dvjvi += 1
                        if topic_word[t][vi] in coherence_corpus[d]:
                            Dvi+=1
                    
                
                    coherence.append(math.log((Dvjvi+epsilon)/(float(Dvj)*float(Dvi)),10))
                    
        print ("average topic: ", ((2/(nTopWords*(nTopWords-1)))*np.sum(coherence))/nTopics)

        return ((2/(nTopWords*(nTopWords-1)))*np.sum(coherence))/nTopics
